...
const useShaderBackground = () => {
  ...
  class WebGLRenderer {
    constructor(canvas, scale) {
      this.canvas = canvas;
      this.scale = scale;
      this.gl = canvas.getContext('webgl2', { alpha: true }); // alpha: true for blending!
    ...
    render(now = 0) {
      ...
      // Transparent clear to allow background to show through!
...
// Adapted to take ReactNode props to preserve custom typography
const AnimatedShaderHero = ({
  ...
  return (
    <div className={`relative w-full min-h-screen flex items-center justify-center overflow-hidden ${className}`}>
      {/* Background canvas blended into global background */}
      <canvas
        ref={canvasRef}
        className="absolute inset-0 w-full h-full object-contain touch-none opacity-40 mix-blend-screen"
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center pt-16">
        ...
        <div className="text-center space-y-6 max-w-5xl mx-auto px-6 w-full relative z-20">
          {children}
          
          {/* CTA Buttons */}
          {buttons && (
            <div className="flex flex-wrap gap-4 justify-center mt-10 animate-fade-in-up animation-delay-800">
              {buttons.primary && (
                <LiquidButton 
...
void main(void) {
    ...
    float alpha = min(1.0, length(uv) * 1.5);
	O=vec4(col, 1.0);
}`;
